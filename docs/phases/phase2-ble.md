...
- Custom BLE service with API characteristics for command/response communication
- Dedicated characteristics for telemetry fields where no standard service exists (e.g., motor power)
- Standard Bluetooth SIG services (cycling, battery, device info)
- Connection management and security
- Data streaming via dedicated telemetry characteristics with configurable update rates
...
├── OpenRideDash Custom Telemetry Service (Custom UUID)
│   ├── Motor Power (notify) - custom telemetry field (no standard service)
│   └── Other custom telemetry characteristics as needed
...
**Note:** The custom telemetry service provides flexibility for different e-bike controllers. Some controllers provide fields like motor power, motor temperature, etc., while others do not. The set of available telemetry characteristics may vary depending on the connected controller type, allowing the system to adapt to different hardware capabilities.
...
5. App subscribes to Event TX for command responses and system events
6. App subscribes to Custom Telemetry characteristics for telemetry data
...
public:
    void update() {
        ...
        // Send telemetry via dedicated telemetry characteristic (not via API service)
...
_Phase 2 enables wireless connectivity, opening up possibilities for mobile integration, remote configuration, and expanded sensor networks while maintaining the reliability established in Phase 1._
